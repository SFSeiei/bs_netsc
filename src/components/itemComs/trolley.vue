<template>
	...
	<div id="trolley" class="col-md-10 ">
					...
					<div class="tab-pane active" id="selectType">
						...
						<form role="form" class="p-4 table-responsive" @submit.prevent=''>
							...
							<div id="addButton" class="text-right">
								...
								<button class="btn btn-danger float-right mr-4" data-toggle="modal" data-target="#trolleyInfo" @click="">提交订单</button>
					...
					<div class="modal-content">
						...
						<div class="modal-footer align-content-center">
							<div class="footerButton text-center">
								...
								<input  ref="submitOrderButton" id="b1" type="submit" class="btn btn-primary mt-2" @click="submitOrder" value="确认提交" data-dismiss="modal">
		...
		<!-- 按钮：用于打开选择付款模态框 -->
		<button ref="selectPayButton" type="button" class="btn btn-primary" data-toggle="modal" data-target="#selectPayModal" style="display:none; ">
			打开选择付款模态框
		</button>
			...
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					...
					<div class="modal-header">
						...
						<button type="button" class="close" data-dismiss="modal" @click="windowReload">&times;</button>
					</div>
						...
						<form role="form" class="p-4 border" @submit.prevent=''>
							<div class="form-inline mt-3 mb-3 r">
								<div id="selectButtonGroup">
									<button ref="" type="button" class="btn btn-danger btn-lg border mr-5" @click="pay" data-dismiss="modal" v-if="">
										立即付款
									</button>
									<button ref="" type="button" class="btn btn-secondary btn-lg border ml-3 mr-5" @click="windowReload" data-dismiss="modal" v-if="">
										稍后付款
									</button>
								</div>
						</div>
				...
				<div class="modal-footer align-content-center">
					<div class="footerButton text-center">
						<button ref="" type="button" class="btn btn-secondary border mt-5 mr-5" @click="windowReload" data-dismiss="modal" v-if="">
							关闭
						</button>
					</div>
				</div>
				...
				<div class="modal-body">
					<form role="form" class="p-4 border" @submit.prevent=''>
						...
						<div class="form-inline mt-3 mb-3">
							...
							<label class="ml-3">
								<span class="text-danger">{{pSum}}</span>元
							</label>
						</div>
						<div class="form-inline mt-3 mb-3">
							<label>
...
<!-- 按钮：用于打开付钱模态框 -->
<button ref="addAddressButton" type="button" class="btn btn-primary" data-toggle="modal" data-target="#addAddressModal" style="display:none; ">
	打开添加地址模态框
</button>
<!-- start pay  modal(模态框) -->
<div class="modal fade" id="addAddressModal">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<div class="form mt-3 mb-3">
					...
					<div class="form-inline mt-3 mb-3">
						...
						<input type="text" class="form-control ml-3" id="addContactWay" v-model='linkmanTel'/>
					</div>					
				</div>
			</div>
			<!-- 模态框底部 -->
			<div class="modal-footer align-content-center">
				<div class="footerButton text-center">
					<button ref="" type="button" class="btn btn-danger border mt-5 mr-5" @click="addAddress" data-dismiss="modal" v-if="">
						添加
					</button>
				</div>
			</div>
		...
		methods:{
        		...
        		if(this.checkboxList.length===0){
        			...
        			if (cont ==true) {
        				...
        				for(var i = 0; i < this.dest.length; i++){
        					...
        					let data = {"getAddress":this.address.addName,"linkmanName":this.address.linkmanName,"telphone":this.address.telphone,"mprice":sum.toFixed(2),"ostate":"未付款","uid":this.userId,"sid":this.dest[i].sId,"getGWay":"配送"}
        					this.addOrders(data);  //因为axios是异步过程，所以不能放在for循环中，另写方法addOrders(data)。
        					for(var k = 0; k < this.dest[i].data.length; k++){
        						// console.log(this.dest[i].data[j].pName)
        						var pro = this.dest[i].data[k]; //因为axios是异步过程，所以不能放在for循环中，另写方法addOrderInfo(pro)。
        						this.addOrderInfo(pro)
        					}
        				}
        	...
        	addOrderInfo(pro){
        		this.$axios.get('/netsc/order/uid='+this.userId).then((res)=>{
        			...
        			let data = {"orderId":res.data.result[res.data.result.length-1].orderId,"pid":pro.pId,"pname":pro.pName,"pnumber":pro.pNumber,"pprice":pro.pPrice}
        			console.log(data);
        	...
        	pay(){
        		this.$axios.get('/netsc/wallet/uid='+this.userId).then((res)=>{
        			...
        			if (res.data.result == null) {
        				...
        				this.lastMoney = (res.data.result.money-this.pSum).toFixed(2);
        			}
        		})
        	},
        	payPass(){
